...
class ShowPrincipalClient_Case(SimpleDistCC_Case):
    """Test handling of --show-principal client option."""

    def runtest(self):
        result, out, err = self.runcmd_unchecked("distcc --version")
...
class AuthEnabledClient_Case(CompileHello_Case):
    ...
    The compile will fail as we don't set up the daemon to
    ...
    def runtest(self):
        ...
        CompileHello_Case.compile(self)
        log = open("distcc.log").read()
        self.assert_re_search('Performing authentication.', log)

    """Override the compileCmd method to use fallback."""
    def compileCmd(self):
        """Return command to compile source"""
        return self.distcc_with_fallback() + \
               _gcc + " -o testtmp.o " + self.compileOpts() + \
...
class UseBlacklist_Case(WithDaemon_Case):
    """Test handling of --blacklist daemon option."""

    """Use an empty blacklist file."""
    def setup(self):
        f = open('list.txt', 'w')
        f.write("""""")
        f.close()
        self.blacklist = os.path.join(os.getcwd(), 'list.txt')
        WithDaemon_Case.setup(self)

    def runtest(self):
        cmd = self.daemon_command()
        log = open("distccd.log").read()
        self.assert_re_search('Using file %s as a blacklist.' % (self.blacklist), log)

    """Override the daemon_command method to use --auth and --blacklist."""
    def daemon_command(self):
        """Return command to start the daemon"""
        return (self.distccd() +
                "--auth --blacklist=%s --verbose --lifetime=%d --daemon "
                "--log-file %s  --pid-file %s --port %d --allow 127.0.0.1"
                % (self.blacklist,
                   self.daemon_lifetime(),
                   _ShellSafe(self.daemon_logfile),
                   _ShellSafe(self.daemon_pidfile),
    ...
    """Override the startDaemon method to handle GSS-API errors."""
    def startDaemon(self):
        """Start a daemon in the background, return its pid"""
        # The daemon detaches once it has successfully bound the
        # socket, so if something goes wrong at startup we ought to
        ...
        try:
          while 1:
            ...
            if result == 0:
                ...
                self.server_port += 1
                continue
            elif result == EXIT_BAD_ARGUMENTS:
                raise comfychair.NotRunError("distcc/distccd not built with GSS-API support.")
            elif result == EXIT_GSSAPI_FAILED:
                raise comfychair.NotRunError("Security mechanism not present.")
            else:
                self.fail("Test for GSS-API/security mechanism failed with %s." % result)
          self.add_cleanup(self.killDaemon)
        finally:
          os.environ['TMPDIR'] = old_tmpdir
          os.chdir("..")
...
class MutualAuthentication_Case(CompileHello_Case):
    ...
    def getHostName(self):
        if not self.hostname:
            ...
            pos = line.find("\n")
            self.hostname = line[0 : pos]
            return self.hostname
        else:
            return self.hostname

    """Override the setupEnv method to set the authentication environment
    ...
    def runtest(self):
        CompileHello_Case.runtest(self)
        log = open("distcc.log").read()
        self.assert_re_search('Successfully authenticated', log)
        log = open("distccd.log").read()
        self.assert_re_search('Successfully authenticated', log)

    """Override the daemon_command method to use --auth and a wider set of
    addresses for the --allow option.  We can't use the loopback address
    as we have to use the hostname for DISTCC_HOSTS which will resolve to
    its proper IP address."""
    def daemon_command(self):
        """Return command to start the daemon"""
        return (self.distccd() +
                "--auth --verbose --lifetime=%d --daemon --log-file %s "
                "--pid-file %s --port %d --allow 0.0.0.0/0"
                % (self.daemon_lifetime(),
                   _ShellSafe(self.daemon_logfile),
                   _ShellSafe(self.daemon_pidfile),
                   self.server_port))
...
class UseWhitelist_Case(MutualAuthentication_Case):
    """Test handling of --whitelist daemon option.
    We also test for authentication failure here
    as the whitelist is empty."""

    """Use an empty whitelist file."""
    def setup(self):
        f = open('list.txt', 'w')
        f.write("""""")
        f.close()
        self.whitelist = os.path.join(os.getcwd(), 'list.txt')
        MutualAuthentication_Case.setup(self)

    def runtest(self):
        CompileHello_Case.runtest(self)
        log = open("distcc.log").read()
        self.assert_re_search('CRITICAL! Access denied by server.', log)
        log = open("distccd.log").read()
        self.assert_re_search('Using file %s as a whitelist.' % (self.whitelist), log)
        self.assert_re_search('Access denied', log)

    """Override the daemon_command method to use --auth and --whitelist."""
    def daemon_command(self):
        """Return command to start the daemon"""
        return (self.distccd() +
                "--auth --whitelist=%s --verbose --lifetime=%d --daemon "
                "--log-file %s --pid-file %s --port %d --allow 0.0.0.0/0"
                % (self.whitelist,
                    self.daemon_lifetime(),
                   _ShellSafe(self.daemon_logfile),
                   _ShellSafe(self.daemon_pidfile),
...
class ExtractPrincipalFromCache:
    """Class to extract a principal name from the ticket cache.
    The principal name will be cached to avoid rerunning klist."""

    principal = ""

    def getPrincipal(self):
        if not self.principal:
            stdin, stdout, stderr = os.popen3("klist")
            for i in stdout.readlines():
                if "Default principal:" in i:
                    pos = i.find("@")
                    line = i[i.index(":") + 2 : pos]
                    self.principal = line
                    break;
            if not self.principal:
                raise comfychair.NotRunError("No default principal name found.")
            else:
...
class AccessDeniedBlacklist_Case(ExtractPrincipalFromCache, MutualAuthentication_Case):
    """Test that access is denied through a blacklist file.
    Extraction of the principal name is attempted from the ticket cache.
    This will only complete successfully if both the client and daemon
...
fpearson
%s
""" % ExtractPrincipalFromCache.getPrincipal(self))
        f.close()
        self.blacklist = os.path.join(os.getcwd(), 'list.txt')
        MutualAuthentication_Case.setup(self)
    ...
    def runtest(self):
        ...
        self.assert_re_search('CRITICAL! Access denied by server.', log)
        log = open("distccd.log").read()
        self.assert_re_search('Access denied - %s blacklisted.'
                    % ExtractPrincipalFromCache.getPrincipal(self), log)
...
class AccessGrantedWhitelist_Case(ExtractPrincipalFromCache, MutualAuthentication_Case):
    """Test that access is granted through a whitelist file.
    Extraction of the principal name is attempted from the ticket cache.
    This will only complete successfully if both the client and daemon
    can obtain credentials."""
...
""" % ExtractPrincipalFromCache.getPrincipal(self))
    ...
    def runtest(self):
        CompileHello_Case.runtest(self)
        log = open("distcc.log").read()
        self.assert_re_search('Access granted by server.', log)
        log = open("distccd.log").read()
        self.assert_re_search('Access granted - %s whitelisted.'
            % ExtractPrincipalFromCache.getPrincipal(self), log)
...
tests = [
         ...
         Getline_Case,
         ShowPrincipalClient_Case,
         ShowPrincipalDaemon_Case,
         AuthEnabledClient_Case,
         AuthEnabledDaemon_Case,
         UseBlacklist_Case,
         UseWhitelist_Case,
         MutualAuthentication_Case,
         AccessDeniedBlacklist_Case,
         AccessGrantedWhitelist_Case,
