...
Options:
 ...
 --gc_threshold              How often to garbage collect (default: 10000).
...
def _ParseCommandLineOptions():
  ...
  for opt, arg in opts:
	...
	basics.opt_debug_pattern = int(arg)
      ...
      if opt in ("--gc_threshold",):
        basics.opt_gc_threshold = int(arg)
...
def Main():
  ...
  if pid != 0: 
    ...
    try:
      try:
        # It has been recommended in various public comments about Python
        # optimization to do garbage collection only rarely. This strategy does
        # not seem to significantly effect the size of process. (Perhaps because
        # reference counting, which is always in effect, takes care of almost
        # all garbage.)
        gc.set_threshold(basics.opt_gc_threshold)
