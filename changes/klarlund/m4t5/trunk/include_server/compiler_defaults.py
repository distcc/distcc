...
def _SystemSearchdirsGCC(compiler, language, canonical_lookup):
  ...
  match_obj = re.search(
    ...
    % ("#include <...> search starts here:",
       "End of search list[.]",
       r"GNU .*? version ((\d|[.])+)"),  # use .*? to pick up nearest version
                                         # match (not that of the linker)
    out,
  ...
  # The include search directories comprise both compiler specific headers, such
  # as those in /usr/lib/gcc/i486-linux-gnu/4.0.3/include, and headers that
  # pertain to the system (such as /usr/lib or /usr/local/lib).  The compiler
  # specific headers are not usually shipped to the compilation servers. We
  # gather the set of such headers, by identifying them as the directories in
  # the search list that contains the version number as one of thee components
  # in their path along with an occurrence of 'gcc' somewhere.
  directories = match_obj.group(1).split()
  version = match_obj.group(2)
  canonicalized_directories = []
  for directory in directories:
    parts = directory.split("/")
    if version in parts and 'gcc' in directory:
      canonicalized_directories.append(canonical_lookup(directory))
  return canonicalized_directories
