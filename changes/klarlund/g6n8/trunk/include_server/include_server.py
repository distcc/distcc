...
Options:
 ...
 --send_systemdirs           Usually system files (those headers that are part
                             of the compiler installation) are not sent to the
                             server.  With this flag a system directory may be
                             specified as in '-isystem /usr/path/lib'
                             (presumably with -nostdinc) and files needed in it
                             will be sent and mounted under the temporary server
                             root.  This option was invented in order to allow
                             distcc pump-mode build the Linux kernel.
...
def _ParseCommandLineOptions():
  ...
  for opt, arg in opts:
	...
	basics.opt_debug_pattern = int(arg)
      ...
      if opt in ("--send_systemdirs",):
        basics.opt_send_systemdirs = True
